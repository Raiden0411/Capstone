...
>
    ...
    <div class="relative flex flex-col h-full max-h-full">
        ...
        <div class="py-2.5 px-4 flex justify-between items-center gap-x-2">
            <div class="-ms-2 flex items-center gap-x-1" x-show="!minified">
                <a class="flex items-center gap-2 font-bold text-xl text-gray-900 dark:text-white"
                   href="{{ route('superadmin.dashboard') }}" wire:navigate aria-label="Brand">
        ...
        <div class="border-t border-gray-200 dark:border-white/10 p-4">
            @auth
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1 min-w-0" x-show="!minified">
                        ...
                        <p class="text-xs text-gray-500 dark:text-white/50 truncate">{{ auth()->user()->email }}</p>
                    </div>
            ...
            @else
                <a href="{{ route('login') }}" class="flex items-center justify-center gap-2 w-full py-2 px-3 text-sm font-medium rounded-lg bg-brand-600 text-white hover:bg-brand-500 transition-colors">Sign in</a>
            @endauth
        </div>
    </div>
</div>