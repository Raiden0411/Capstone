  ...
  <nav class="max-w-[85rem] w-full mx-auto px-4 flex flex-wrap basis-full items-center justify-between" aria-label="Global">
    ...
    <div class="flex items-center gap-x-6 ms-auto lg:order-3">
      ...
      <div class="flex items-center gap-x-3">
        ...
        @guest
          <a href="{{ route('login') }}" wire:navigate class="py-2 px-6 inline-flex items-center text-xs font-bold tracking-[0.1em] uppercase rounded-full bg-yellow-400 border border-yellow-400 text-[#062c1f] hover:bg-transparent hover:text-yellow-400 transition-all duration-300">
            Sign in
          </a>
        @endguest
        ...
        @auth
          <div class="hs-dropdown relative inline-flex">
            ...
            <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-[200px] bg-[#062c1f] border border-emerald-700/50 rounded-xl p-2 mt-2 shadow-xl z-50">
              <a class="flex items-center gap-x-3 py-2 px-3 rounded-lg text-sm text-emerald-100/80 hover:bg-emerald-900/40 hover:text-yellow-400 transition-colors" href="{{ route('explore.map') }}">
