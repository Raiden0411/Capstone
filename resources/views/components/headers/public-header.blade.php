<div>
    ...
    <header class="fixed top-0 left-0 right-0 z-50 flex flex-wrap sm:justify-start sm:flex-nowrap w-full bg-[#062c1f] dark:bg-[#0b0f19] border-b border-emerald-700/50 dark:border-slate-700/50 text-sm py-4 transition-colors duration-300 h-[64px]"
        ...
        <nav class="max-w-[85rem] w-full mx-auto px-4 flex flex-wrap basis-full items-center justify-between" aria-label="Global">
            {{-- Logo --}}
            <a class="flex flex-col group" href="{{ route('home') }}" wire:navigate>
            ...
            <div class="flex items-center gap-x-6 ms-auto">
                ...
                <div class="hidden lg:flex lg:items-center lg:gap-x-8">
                    <a class="relative text-xs font-bold tracking-[0.15em] uppercase transition-all duration-300 {{ request()->routeIs('home') ? 'text-yellow-400 after:absolute after:bottom-[-4px] after:left-0 after:h-[2px] after:w-full after:bg-yellow-400' : 'text-emerald-100/80 dark:text-slate-300 hover:text-yellow-400 dark:hover:text-white hover:translate-y-[-1px]' }}" href="{{ route('home') }}" wire:navigate>Home</a>
                    <a class="text-xs font-bold tracking-[0.15em] uppercase transition-all duration-300 {{ request()->routeIs('about') ? 'text-yellow-400 after:absolute after:bottom-[-4px] after:left-0 after:h-[2px] after:w-full after:bg-yellow-400' : 'text-emerald-100/80 dark:text-slate-300 hover:text-yellow-400 dark:hover:text-white hover:translate-y-[-1px]' }}" href="{{ route('about') }}" wire:navigate>About</a>
                    <a class="text-xs font-bold tracking-[0.15em] uppercase text-emerald-100/80 dark:text-slate-300 hover:text-yellow-400 dark:hover:text-white hover:translate-y-[-1px] transition-all duration-300" href="{{ route('explore.map') }}" wire:navigate>Explore</a>
                </div>

                <div class="flex items-center gap-x-3">
                    <a href="{{ route('explore.map') }}" wire:navigate class="hidden sm:inline-flex py-2 px-5 items-center text-xs font-bold tracking-[0.1em] uppercase rounded-full text-emerald-100 dark:text-slate-300 border border-emerald-700/50 dark:border-slate-600 hover:border-yellow-400 dark:hover:border-blue-400 hover:text-yellow-400 dark:hover:text-white transition-all duration-300">
                        Places
                    </a>

                    {{-- Dark Mode Toggle (same Alpine logic as admin) --}}
                    <button type="button"
                    ...
                    @guest
                        <a href="{{ route('login') }}" wire:navigate class="py-2 px-6 inline-flex items-center text-xs font-bold tracking-[0.1em] uppercase rounded-full bg-yellow-400 dark:bg-blue-600 border border-yellow-400 dark:border-blue-600 text-[#062c1f] dark:text-white hover:bg-transparent dark:hover:bg-blue-700 hover:text-yellow-400 dark:hover:text-white transition-all duration-300">
                            Sign in
                        </a>
                    @endguest
                    ...
                    @auth
                        ...
                        <div class="relative" @click.outside="userDropdownOpen = false">
                            ...
                            <div x-cloak x-show="userDropdownOpen" x-transition
                                ...
                                <a class="flex items-center gap-x-3 py-2 px-3 rounded-lg text-sm text-emerald-100/80 dark:text-slate-300 hover:bg-emerald-900/40 dark:hover:bg-slate-700 hover:text-yellow-400 dark:hover:text-white transition-colors" href="{{ route('explore.map') }}" wire:navigate>
                                ...
                                {{-- NEW: My Bookings link (pointing to the tourist booking history) --}}
                                <a class="flex items-center gap-x-3 py-2 px-3 rounded-lg text-sm text-emerald-100/80 dark:text-slate-300 hover:bg-emerald-900/40 dark:hover:bg-slate-700 hover:text-yellow-400 dark:hover:text-white transition-colors" href="{{ route('my-bookings') }}" wire:navigate>
            ...
            <div x-cloak x-show="mobileOpen" x-transition
                ...
                <a class="block text-xs font-bold tracking-[0.15em] uppercase {{ request()->routeIs('home') ? 'text-yellow-400 dark:text-white' : 'text-emerald-100/80 dark:text-slate-300' }}" href="{{ route('home') }}" wire:navigate>Home</a>
                <a class="block text-xs font-bold tracking-[0.15em] uppercase {{ request()->routeIs('about') ? 'text-yellow-400 dark:text-white' : 'text-emerald-100/80 dark:text-slate-300' }}" href="{{ route('about') }}" wire:navigate>About</a>
                <a class="block text-xs font-bold tracking-[0.15em] uppercase text-emerald-100/80 dark:text-slate-300" href="{{ route('explore.map') }}" wire:navigate>Explore</a>
                @auth
                    {{-- NEW: My Bookings link in mobile menu --}}
                    <a class="block text-xs font-bold tracking-[0.15em] uppercase text-emerald-100/80 dark:text-slate-300" href="{{ route('my-bookings') }}" wire:navigate>My Bookings</a>
                @endauth
            </div>
        </nav>
    </header>

    {{-- This spacer pushes page content below the fixed header --}}
