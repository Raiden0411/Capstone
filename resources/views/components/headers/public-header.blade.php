<div>
    <header
        ...
        <nav class="max-w-[85rem] w-full mx-auto px-8 flex items-center justify-between">
            ...
            {{-- Logo --}}
            <a class="flex flex-col gap-[2px] group" href="{{ route('home') }}" wire:navigate>
            ...
            <div class="hidden lg:flex items-center gap-10">
                <a href="{{ route('home') }}" wire:navigate
                   class="relative text-[11px] font-medium tracking-[0.16em] uppercase transition-colors duration-200 {{ request()->routeIs('home') ? 'text-amber-300 after:absolute after:bottom-[-4px] after:left-0 after:right-0' : 'text-white/45 hover:text-white/85' }}">Home</a>
                <a href="{{ route('about') }}" wire:navigate
                   class="text-[11px] font-medium tracking-[0.16em] uppercase transition-colors duration-200 {{ request()->routeIs('about') ? 'text-amber-300' : 'text-white/45 hover:text-white/85' }}">About</a>
            ...
            <div class="flex items-center gap-2.5">
                ...
                <a href="{{ route('learnmore') }}" wire:navigate
                ...
                @auth
                    {{-- ✅ Updated user button with avatar (Alpine reactive, using Storage::url) --}}
                    <div class="relative" @click.outside="userDropdownOpen = false">
                        ...
                        <div x-cloak x-show="userDropdownOpen" x-transition
                            ...
                            {{-- ✅ User info with avatar (Alpine reactive) --}}
                            ...
                            <div class="p-1.5 space-y-0.5">
                                <a class="flex items-center gap-3 py-2 px-3 rounded-lg text-sm text-white/55 hover:bg-white/[0.05] hover:text-amber-300 transition-colors"
                                   href="{{ route('explore.map') }}" wire:navigate>
        ...
        <div x-cloak x-show="mobileOpen" x-transition
            ...
            <a class="block text-[11px] font-medium tracking-[0.16em] uppercase {{ request()->routeIs('home') ? 'text-amber-300' : 'text-white/45' }}" href="{{ route('home') }}" wire:navigate>Home</a>
            <a class="block text-[11px] font-medium tracking-[0.16em] uppercase {{ request()->routeIs('about') ? 'text-amber-300' : 'text-white/45' }}" href="{{ route('about') }}" wire:navigate>About</a>
            <a class="block text-[11px] font-medium tracking-[0.16em] uppercase text-white/45" href="{{ route('explore.map') }}" wire:navigate>Explore</a>
            @auth
                <a class="block text-[11px] font-medium tracking-[0.16em] uppercase text-white/45" href="{{ route('my-bookings') }}" wire:navigate>My Bookings</a>
                <a class="block text-[11px] font-medium tracking-[0.16em] uppercase text-white/45" href="{{ route('profile') }}" wire:navigate>My Profile</a>
            @endauth
    ...
    {{-- ❌ REMOVED the old spacer <div class="pt-[68px]"></div> --}}
</div>