  ...
  <header class="sticky top-0 inset-x-0 flex flex-wrap md:justify-start md:flex-nowrap z-30 w-full bg-black/60 backdrop-blur-xl border-b border-white/10 text-sm py-2.5 transition-all duration-300"
    ...
    <nav class="px-4 sm:px-6 flex basis-full items-center w-full mx-auto justify-between">
      ...
      {{-- Left side: Only mobile sidebar toggle (no brand) --}}
      ...
      {{-- Right side: Dark mode toggle + Profile Dropdown (only identity) --}}
      <div class="flex items-center gap-2 ms-auto">
        ...
        @auth
          ...
          <div class="relative"
            ...
            {{-- Trigger – logo if exists, else user initial --}}
            <button type="button"
                    @click="open = !open"
        ...
        @else
          <a href="{{ route('login') }}" class="py-2 px-4 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg bg-brand-600 text-white hover:bg-brand-500">Sign in</a>
        @endauth
      </div>
    </nav>
  </header>