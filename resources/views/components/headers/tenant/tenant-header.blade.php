...
<header class="sticky top-0 inset-x-0 flex flex-wrap md:justify-start md:flex-nowrap z-30 w-full bg-white dark:bg-[#0b0f19] border-b border-gray-200 dark:border-slate-700/50 text-sm py-2.5 transition-all duration-300"
  ...
  <nav class="px-4 sm:px-6 flex basis-full items-center w-full mx-auto justify-between">
    ...
    <div class="flex items-center gap-2 lg:hidden me-5">
      ...
      <a class="flex-none font-bold text-xl text-gray-900 dark:text-white" href="{{ route('tenant.dashboard') }}" wire:navigate aria-label="Tenant Dashboard">
        {{-- You can replace with a logo image --}}
        ...
        <span class="text-blue-600 dark:text-blue-400">{{ auth()->user()->tenant?->name ?? 'My Business' }}</span>
      </a>
    </div>
    ...
    <div class="flex items-center gap-2 ms-auto">
      ...
      @else
        <a href="{{ route('login') }}" class="py-2 px-4 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg bg-blue-600 text-white hover:bg-blue-700">Sign in</a>
      @endauth
    </div>
  </nav>
</header>