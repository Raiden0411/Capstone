...
class extends Component {
    ...
    public function login()
    {
        ...
        if (Auth::attempt(['email' => $this->email, 'password' => $this->password], $this->remember)) {
            ...
            // Business Owner (tenant admin) → tenant dashboard
            if ($user->hasRole('admin')) {
                return $this->redirectRoute('tenant.dashboard', navigate: true);
            }

            // Employee / staff (belongs to a tenant and has permissions) → employee dashboard
            if ($user->tenant_id && $user->getAllPermissions()->count() > 0) {
                return $this->redirectRoute('tenant.employee.dashboard', navigate: true);
            }

            // Tourist or unprivileged user → public home
...
{{-- The rest of the view (glass card, illustration, form) stays exactly the same --}}
    ...
    <div class="w-full max-w-4xl glass-card flex flex-col lg:flex-row overflow-hidden !rounded-3xl !p-0">
        ...
        <div class="w-full lg:max-w-md p-8 sm:p-10 relative bg-black/30 backdrop-blur-sm">
            ...
            <p class="mt-1 text-sm text-white/60">
                New here?
                <a href="{{ route('register') }}" class="font-medium text-brand-400 decoration-2 hover:underline focus:outline-none focus:underline">Create an account</a>
            </p>
            <p class="mt-1 text-sm text-white/60">
                Own a tourist spot?
                <a href="{{ route('register_business') }}" wire:navigate class="font-medium text-brand-400 decoration-2 hover:underline focus:outline-none focus:underline">Register your business</a>
            </p>
