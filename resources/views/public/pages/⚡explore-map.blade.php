        ...
        <div x-data="{ open: window.innerWidth >= 1024 }"
            ...
            <div x-show="open" x-transition:enter="transition-transform duration-300" x-transition:leave="transition-transform duration-200"
                ...
                <div class="p-5 border-b border-gray-200 dark:border-slate-700/50">
                    ...
                    <a href="{{ route('home') }}" wire:navigate class="text-sm text-blue-600 dark:text-blue-400 hover:underline mt-1 inline-block">&larr; Back to Home</a>
                </div>
                ...
                <div class="flex-1 overflow-y-auto">
                    @forelse($this->tenants as $index => $tenant)
                        <div wire:key="tenant-{{ $tenant->id }}"
                            ...
                            <span class="size-3 rounded-full shrink-0" style="background: hsl({{ ($index * 137) % 360 }}, 65%, 55%); box-shadow: 0 0 6px hsl({{ ($index * 137) % 360 }}, 65%, 55%);"></span>
                            <div class="min-w-0 flex-1">
                                <p class="text-sm font-semibold text-gray-900 dark:text-white truncate">{{ $tenant->name }}</p>
                                <p class="text-xs text-gray-500 dark:text-slate-400">{{ $tenant->typeOfTenant->type ?? 'Business' }}</p>
                            </div>
                            <button onclick="window.requestDirections({{ $index }})"
