...
<div class="min-h-screen bg-slate-50 py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {{-- Page header --}}
        <div class="mb-10">
            <a href="{{ route('tenant.show', $tenant->slug) }}" wire:navigate class="text-blue-600 hover:underline text-sm flex items-center gap-1 mb-4">
                &larr; Back to {{ $tenant->name }}
            </a>
            <h1 class="text-3xl font-bold text-slate-900">{{ $tenant->name }} – What We Offer</h1>
                ...
                @forelse($this->properties as $property)
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-hidden hover:shadow-lg transition-all">
                        ...
                        <div class="p-5 flex-1 flex flex-col">
                            <p class="text-xs font-bold text-blue-600 uppercase mb-1">{{ $property->propertyType->name ?? 'Property' }}</p>
                            <h3 class="font-bold text-xl text-slate-900 mb-2">{{ $property->name }}</h3>
                            <p class="text-sm text-slate-500 mb-4 flex-1">{{ $property->description }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($this->services as $service)
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col">
                        <h3 class="font-bold text-lg text-slate-900 mb-3">{{ $service->name }}</h3>
                        <p class="text-sm text-slate-500 mb-4 flex-1">{{ $service->description ?? 'No description available' }}</p>
                        <div class="border-t border-slate-100 pt-3">
                            <span class="text-2xl font-extrabold text-slate-900">₱{{ number_format($service->price, 2) }}</span>
