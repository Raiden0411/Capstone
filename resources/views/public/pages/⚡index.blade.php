        ...
        return $this->tenants
            ...
            ->map(function ($tenant, $index) {
                return [
                    ...
                    'coordinates' => $tenant->coordinates,   // array of coordinate objects
                ];
            })->values()->toArray();
    }
...
<div>
    <div class="relative z-10">
        ...
        <div class="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid lg:grid-cols-2 gap-12 items-center">
                <div class="space-y-6">
                    ...
                    <div class="pt-4">
                        <a href="{{ route('explore.map') }}" wire:navigate class="inline-flex items-center gap-x-2 py-3 px-6 rounded-full bg-brand-600 hover:bg-brand-500 text-white font-semibold shadow-lg shadow-brand-500/20 transition">
                            Plan Your Visit
                        </a>
                    </div>
                </div>
                ...
                <div data-hs-carousel='{"loadingClasses": "opacity-0", "isAutoPlay": true}' class="relative group rounded-3xl overflow-hidden shadow-2xl">
                    ...
                    @if($this->carouselTenants->count() > 1)
                    ...
                    @foreach($this->tenants as $tenant)
                        ...
                        <div class="group bg-white dark:bg-white/5 dark:backdrop-blur-md border border-gray-200 dark:border-white/10 rounded-3xl overflow-hidden shadow-sm dark:shadow-none hover:shadow-xl dark:hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                            <div class="h-56 overflow-hidden">
                                <img src="{{ $cardImage }}" alt="{{ $tenant->name }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-700" loading="lazy" onerror="this.src='https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?q=80&w=800'">
                            </div>
                            <div class="p-6">
                    ...
                    <div class="lg:col-span-2 pt-2 order-1 lg:order-2">
                        ...
                        <div id="location-list" class="flex flex-col gap-2 mb-8">
                            @foreach($this->mapLocations() as $index => $loc)
                                <button onclick="focusLocation({{ $index }})" class="group flex items-center gap-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl px-4 py-3 cursor-pointer transition-all duration-200 text-left w-full focus:outline-none focus:ring-1 focus:ring-brand-400/50">
                                    ...
                                    <span class="text-[11px] text-white/40 font-medium shrink-0">{{ $loc['type'] }}</span>
                                </button>
                            @endforeach
                        </div>
                        ...
                        <a href="{{ route('explore.map') }}" wire:navigate class="inline-flex items-center gap-x-2 py-3 px-6 text-sm font-semibold rounded-full bg-white dark:bg-brand-600 text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-brand-500 transition-colors shadow-lg shadow-brand-500/20">
                            Explore Full Map
        ...
        <section class="relative py-24 overflow-hidden bg-gray-50 dark:bg-white/5">
            ...
            <div class="relative max-w-4xl mx-auto px-4 text-center">
                ...
                <a href="{{ route('explore.map') }}" wire:navigate class="inline-flex items-center gap-x-2 py-3 px-8 text-sm font-semibold rounded-full bg-brand-600 hover:bg-brand-500 text-white shadow-lg shadow-brand-500/20 transition">
                    Explore Now
                    <svg class="size-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3"/></svg>
                </a>
            </div>
    ...
    {{-- Scripts pushed into the layout stack --}}
    @push('scripts')
    <script>
        // Carousel fix
