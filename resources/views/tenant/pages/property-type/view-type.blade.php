...
class extends Component {
    ...
    public function delete(int $id)
    {
        ...
        // Tenants can only delete their own custom types
...
<div class="p-6 sm:p-10 max-w-6xl mx-auto">
    ...
    <div class="mb-8 flex justify-between items-center">
        ...
        <a href="{{ route('tenant.property-types.create') }}" wire:navigate class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 px-5 rounded-lg shadow-sm transition flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
            ...
            <p class="text-2xl font-bold text-blue-600">{{ $this->stats['custom'] }}</p>
        </div>
    </div>
                    ...
                    @forelse($this->types as $type)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="text-sm text-slate-600">{{ $type->properties_count }}</span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    @if(!is_null($type->tenant_id))
                                        <a href="{{ route('tenant.property-types.edit', $type->id) }}" wire:navigate class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Edit">
