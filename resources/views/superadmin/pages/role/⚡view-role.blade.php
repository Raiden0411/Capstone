...
<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 pb-6 border-b border-gray-200 dark:border-white/10">
        ...
        <a href="{{ route('superadmin.roles.create') }}" wire:navigate class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-500 text-white text-sm font-semibold shadow-lg shadow-brand-500/20 transition hover:scale-105">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5 text-gray-700 dark:text-white/80">
                    @forelse($this->roles as $role)
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                            <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-white/40 font-mono">#{{ $role->id }}</td>
                                            ...
                                            <div class="absolute z-10 left-0 mt-2 w-64 p-3 bg-white dark:bg-white/10 dark:backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                                                ...
                                                <div class="flex flex-wrap gap-1 max-h-40 overflow-y-auto">
                                                    @foreach($role->permissions->take(15) as $permission)
                                                        <span class="text-xs bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-white/70 px-2 py-0.5 rounded">{{ $permission->name }}</span>
                                                    @endforeach
                                                    @if($role->permissions->count() > 15)
                                                        <span class="text-xs text-gray-400 dark:text-white/40">+{{ $role->permissions->count() - 15 }} more</span>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                @if($role->name !== 'super-admin')
                                    <a href="{{ route('superadmin.roles.edit', $role->id) }}" wire:navigate class="text-brand-600 dark:text-brand-400 hover:text-brand-500 dark:hover:text-brand-300 mr-3 transition-colors inline-flex items-center gap-1">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        Edit
                                    </a>
                                    <button wire:click="delete({{ $role->id }})" wire:confirm="Are you sure you want to delete this role? Any users assigned to this role will lose their permissions." class="text-red-500 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 transition-colors inline-flex items-center gap-1">
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <span class="text-gray-500 dark:text-white/40">No roles found{{ $search ? ' matching "' . $search . '"' : '' }}.</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
