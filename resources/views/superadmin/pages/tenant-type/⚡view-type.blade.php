...
<div class="max-w-5xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 pb-6 border-b border-gray-200 dark:border-white/10">
        ...
        <a href="{{ route('superadmin.tenant-types.create') }}" wire:navigate class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-500 text-white text-sm font-semibold shadow-lg shadow-brand-500/20 transition hover:scale-105">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5 text-gray-700 dark:text-white/80">
                    @forelse($this->types as $type)
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                            ...
                            <td class="px-4 sm:px-6 py-4 hidden sm:table-cell text-gray-600 dark:text-white/60">{{ $type->description ?? '—' }}</td>
                            ...
                            <td class="px-4 sm:px-6 py-4 text-right whitespace-nowrap space-x-2">
                                <a href="{{ route('superadmin.tenant-types.edit', $type->id) }}" wire:navigate class="text-brand-600 dark:text-brand-400 hover:text-brand-500 dark:hover:text-brand-300 font-medium transition-colors">Edit</a>
                                <button wire:click="delete({{ $type->id }})" wire:confirm="Delete this tenant type?" class="text-red-500 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 font-medium transition-colors">Delete</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
