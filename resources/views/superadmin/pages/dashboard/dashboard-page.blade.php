...
<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 pb-6 border-b border-gray-200 dark:border-white/10">
        ...
        <div class="text-xs font-mono text-gray-400 dark:text-white/40 text-right space-y-1">
            <div>System time <span class="text-gray-600 dark:text-white/60">{{ now()->format('D, d M Y · H:i') }}</span></div>
            <div>Environment <span class="text-gray-600 dark:text-white/60">{{ app()->environment() }}</span></div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white dark:bg-white/5 dark:backdrop-blur-md border border-gray-200 dark:border-white/10 rounded-2xl p-6 shadow-sm dark:shadow-none relative overflow-hidden">
            ...
            <p class="font-mono text-4xl font-light text-brand-600 dark:text-brand-400 mb-4">{{ $s['total_tenants'] }}</p>
            <div class="flex flex-wrap gap-2 mb-4">
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-mono font-medium bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-500/30">
                    <span class="w-1.5 h-1.5 rounded-full bg-current"></span> {{ $s['active_tenants'] }} active
                </span>
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-mono font-medium bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-500/30">
                    <span class="w-1.5 h-1.5 rounded-full bg-current"></span> {{ $s['pending_tenants'] }} pending
                </span>
            </div>
            <div class="space-y-1">
                <div class="flex justify-between text-xs font-mono text-gray-400 dark:text-white/40">
                    <span>Activation rate</span>
                    <span>{{ $s['total_tenants'] > 0 ? round(($s['active_tenants'] / $s['total_tenants']) * 100) : 0 }}%</span>
                </div>
                <div class="w-full h-1 bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
                    <div class="h-full rounded-full bg-brand-500 transition-all duration-700" style="width:{{ $s['total_tenants'] > 0 ? round(($s['active_tenants'] / $s['total_tenants']) * 100) : 0 }}%"></div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-white/5 dark:backdrop-blur-md border border-gray-200 dark:border-white/10 rounded-2xl p-6 shadow-sm dark:shadow-none relative overflow-hidden">
            ...
            <p class="font-mono text-4xl font-light text-amber-600 dark:text-amber-400 mb-4">{{ $s['pending_tenants'] }}</p>
            <div class="flex flex-wrap gap-2 mb-4">
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-mono font-medium bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-500/30">
            ...
            <div class="space-y-1">
                <div class="flex justify-between text-xs font-mono text-gray-400 dark:text-white/40">
                    ...
                    <span>{{ $s['total_tenants'] > 0 ? round(($s['pending_tenants'] / $s['total_tenants']) * 100) : 0 }}%</span>
                </div>
                <div class="w-full h-1 bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
                    <div class="h-full rounded-full bg-amber-400 transition-all duration-700" style="width:{{ $s['total_tenants'] > 0 ? round(($s['pending_tenants'] / $s['total_tenants']) * 100) : 0 }}%"></div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-white/5 dark:backdrop-blur-md border border-gray-200 dark:border-white/10 rounded-2xl p-6 shadow-sm dark:shadow-none relative overflow-hidden">
            ...
            <p class="font-mono text-4xl font-light text-green-600 dark:text-green-400 mb-4">{{ $s['new_this_week'] }}</p>
            <div class="flex flex-wrap gap-2 mb-4">
            ...
            <div class="space-y-1">
                <div class="flex justify-between text-xs font-mono text-gray-400 dark:text-white/40">
                    ...
                    <span>{{ $s['total_tenants'] > 0 ? round(($s['new_this_week'] / $s['total_tenants']) * 100) : 0 }}%</span>
                </div>
                <div class="w-full h-1 bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
                    <div class="h-full rounded-full bg-green-500 transition-all duration-700" style="width:{{ $s['total_tenants'] > 0 ? round(($s['new_this_week'] / $s['total_tenants']) * 100) : 0 }}%"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-white/5 dark:backdrop-blur-md border border-gray-200 dark:border-white/10 rounded-2xl overflow-hidden shadow-sm dark:shadow-none">
        ...
        <div class="p-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="bg-gray-50 dark:bg-white/5 rounded-xl p-5">
                ...
                <p class="font-mono text-3xl font-light text-gray-900 dark:text-white">{{ $s['total_users'] }}</p>
            </div>
            <div class="bg-gray-50 dark:bg-white/5 rounded-xl p-5">
                <p class="text-xs font-mono uppercase tracking-widest text-gray-400 dark:text-white/40 mb-2">System roles</p>
                <p class="font-mono text-3xl font-light text-gray-900 dark:text-white">{{ $s['total_roles'] }}</p>
            </div>
            <div class="bg-gray-50 dark:bg-white/5 rounded-xl p-5">
                <p class="text-xs font-mono uppercase tracking-widest text-gray-400 dark:text-white/40 mb-2">Total businesses</p>
                <p class="font-mono text-3xl font-light text-gray-900 dark:text-white">{{ $s['total_tenants'] }}</p>
            </div>
        </div>
    </div>
            ...
            @else
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    @foreach($this->recentTenants as $tenant)
                        <div class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/10 transition-colors group" wire:key="t-{{ $tenant->id }}">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs font-mono text-gray-400 dark:text-white/40">{{ $tenant->created_at->diffForHumans() }}</p>
                            </div>
                            <a href="{{ route('superadmin.tenants.edit', $tenant->id) }}" wire:navigate class="shrink-0 text-xs font-mono text-gray-400 dark:text-white/40 group-hover:text-brand-600 dark:group-hover:text-brand-400 border border-gray-300 dark:border-white/10 rounded-md px-2 py-1 hover:border-brand-300 dark:hover:border-brand-400/50 transition-colors">
                                Manage →
                            </a>
                        </div>
                    @endforeach
