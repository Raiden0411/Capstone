    ...
    <div class="relative z-10 w-full max-w-4xl glass-card flex flex-col lg:flex-row overflow-hidden !rounded-3xl !p-0">
        ...
        <div class="w-full lg:max-w-md p-8 sm:p-10 relative bg-black/30 backdrop-blur-sm">
            ...
            <p class="mt-1 text-sm text-white/60">
                New here?
                <a href="<?php echo e(route('register')); ?>" class="font-medium text-brand-400 decoration-2 hover:underline">Create an account</a>
            </p>
            <p class="mt-1 text-sm text-white/60">
                Own a tourist spot?
                <a href="<?php echo e(route('register_business')); ?>" class="font-medium text-brand-400 decoration-2 hover:underline">Register your business</a>
            </p>

            
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($errors->any()): ?>
                <div class="mt-4 text-red-400 text-sm"><?php echo e($errors->first('email')); ?></div>
            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>

            
            <form method="POST" action="<?php echo e(route('login')); ?>" class="space-y-4 mt-6">
                <?php echo csrf_field(); ?>
                <div>
                    <label for="email" class="block text-sm font-medium mb-2 text-white/70">Email Address</label>
                    <input type="email" id="email" name="email" value="<?php echo e(old('email')); ?>" placeholder="example@email.com"
